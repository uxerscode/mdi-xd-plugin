...
async function createDialog({
    ...
    try {
        if (!dialogManifest) {
            dialogManifest = await getManifest();
        }
    } catch (err) {
        // do nothing
    }
    ...
    if (icon === 'plugin-icon') {
        if (dialogManifest.icons) {
            usingPluginIcon = true;
            iconSize = 24;
            icon = getNearestIcon(dialogManifest, iconSize);
        }
    }
...
<form method="dialog">
    <h1 class="h1">
        ...
        ${icon ? `<img ${usingPluginIcon ? `class="plugin-icon" title="${dialogManifest.name}"` : ''} src="${icon}" />` : ''}
    </h1>
    <hr />
    <div class="container">
        ${
